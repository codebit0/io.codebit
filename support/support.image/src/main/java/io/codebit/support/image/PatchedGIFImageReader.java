...
public class PatchedGIFImageReader extends GIFImageReader {
    ...
    public void initializeStringTable(int[] prefix,
        ...
        for (int i = numEntries; i < prefix.length; i++) {
            prefix[i] = -1;
            length[i] = 1;
        }
    ...
    public BufferedImage read(int imageIndex, ImageReadParam param)
        ...
        try {
            ...
            int[] prefix = new int[4096+1024];
            byte[] suffix = new byte[4096+1024];
            byte[] initial = new byte[4096+1024];
            int[] length = new int[4096+1024];
            ...
            while (!abortRequested()) {
                ...
                if (code == clearCode) {
                    ...
                    } catch (ArrayIndexOutOfBoundsException e)
                    {
                    	...
                    	prefix = Arrays.copyOf(prefix, prefix.length+1024);
                    	suffix = Arrays.copyOf(suffix, suffix.length+1024);
                    	initial = Arrays.copyOf(initial, initial.length+1024);
                    	length = Arrays.copyOf(length, length.length+1024);
                    	initializeStringTable(prefix, suffix, initial, length);
                    	continue;
//                    	//Die.
